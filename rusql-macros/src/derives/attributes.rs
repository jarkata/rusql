...
pub struct TypeName {
    pub val: String,
    pub span: Span,
    /// Whether the old rusql(rename) syntax was used instead of rusql(type_name)
...
pub struct RuSqlContainerAttributes {
...
pub struct RuSqlChildAttributes {
...
pub fn parse_container_attributes(input: &[Attribute]) -> syn::Result<RuSqlContainerAttributes> {
    ...
    for attr in input
        .iter()
        .filter(|a| a.path.is_ident("rusql") || a.path.is_ident("repr"))
    {
        let meta = attr
            .parse_meta()
            .map_err(|e| syn::Error::new_spanned(attr, e))?;
        match meta {
            Meta::List(list) if list.path.is_ident("rusql") => {
...
pub fn parse_child_attributes(input: &[Attribute]) -> syn::Result<RuSqlChildAttributes> {
    let mut rename = None;
    let mut default = false;
    let mut try_from = None;
    let mut flatten = false;

    for attr in input.iter().filter(|a| a.path.is_ident("rusql")) {
        let meta = attr
            .parse_meta()
            .map_err(|e| syn::Error::new_spanned(attr, e))?;
...
pub fn check_enum_attributes(input: &DeriveInput) -> syn::Result<RuSqlContainerAttributes> {
    let attributes = parse_container_attributes(&input.attrs)?;

    assert_attribute!(
        !attributes.transparent,
        "unexpected #[rusql(transparent)]",
        input
    );

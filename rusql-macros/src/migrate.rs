...
use rusql_core::migrate::MigrationType;
use std::fs;
use std::path::Path;
use syn::LitStr;
...
impl ToTokens for QuotedMigrationType {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        let ts = match self.0 {
            MigrationType::Simple => quote! { ::rusql::migrate::MigrationType::Simple },
            MigrationType::ReversibleUp => quote! { ::rusql::migrate::MigrationType::ReversibleUp },
            MigrationType::ReversibleDown => {
                quote! { ::rusql::migrate::MigrationType::ReversibleDown }
            }
        };
...
// mostly copied from rusql-core/src/migrate/source.rs
...
pub(crate) fn expand_migrator(path: &Path) -> crate::Result<TokenStream> {
    ...
    #[cfg(any(rusql_macros_unstable, procmacro2_semver_exempt))]
    {

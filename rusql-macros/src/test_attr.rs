...
fn expand_simple(input: syn::ItemFn) -> TokenStream {
    ...
    quote! {
        ...
        #(#attrs)*
        fn #name() #ret {
            ::rusql::test_block_on(async { #body })
        }
    }
}
...
fn expand_advanced(args: syn::AttributeArgs, input: syn::ItemFn) -> crate::Result<TokenStream> {
    ...
    Ok(quote! {
        ...
        fn #name() #ret {
            ...
            let mut args = ::rusql::testing::TestArgs::new(concat!(module_path!(), "::", stringify!(#name)));
            ...
            ::rusql::testing::TestFn::run_test(f, args)
        }
    })
}

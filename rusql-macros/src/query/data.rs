...
use rusql_core::executor::Executor;
...
pub mod offline {
    ...
    impl DynQueryData {
        /// Find and deserialize the data table for this query from a shared `rusql-data.json`
        ...
        pub fn from_data_file(path: impl AsRef<Path>, query: &str) -> crate::Result<Self> {
            ...
            #[cfg(procmacr2_semver_exempt)]
            {
                ...
                let path = path.to_str().ok_or_else(|| {
                    format!(
                        "rusql-data.json path cannot be represented as a string: {:?}",
                        path
                    )
                })?;

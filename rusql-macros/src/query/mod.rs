...
use rusql_core::connection::Connection;
use rusql_core::database::Database;
use rusql_core::{column::Column, describe::Describe, type_info::TypeInfo};
use rusql_rt::{block_on, AsyncMutex};
...
pub fn expand_input(input: QueryMacroInput) -> crate::Result<TokenStream> {
    match &*METADATA {
        ...
        #[cfg(feature = "offline")]
        _ => {
            let data_file_path = METADATA.manifest_dir.join("rusql-data.json");

            if data_file_path.exists() {
                expand_from_file(input, data_file_path)
            } else {
                let workspace_data_file_path = METADATA.workspace_root().join("rusql-data.json");
                if workspace_data_file_path.exists() {
                    expand_from_file(input, workspace_data_file_path)
                } else {
                    Err(
                        "`DATABASE_URL` must be set, or `cargo rusql prepare` must have been run \
                     and rusql-data.json must exist, to use query macros"
                            .into(),
                    )
                }
            }
        ...
        Metadata { offline: true, .. } => {
            Err("The cargo feature `offline` has to be enabled to use `RUSQL_OFFLINE`".into())
        }
...
fn expand_from_db(input: QueryMacroInput, db_url: &str) -> crate::Result<TokenStream> {
    use rusql_core::any::{AnyConnectOptions, AnyConnection};
    ...
    // - https://github.com/launchbadge/rusql/pull/1782#issuecomment-1089226716
    // - https://github.com/launchbadge/rusql/issues/1929
    #[cfg(feature = "sqlite")]
    if let Some(sqlite_opts) = connect_opts.as_sqlite() {
        ...
        let describe = rusql_core::sqlite::describe_blocking(sqlite_opts, &input.sql)?;
    ...
    block_on(async {
        ...
        match conn_item.private_get_mut() {
            ...
            #[cfg(feature = "mysql")]
            rusql_core::any::AnyConnectionKind::MySql(conn) => {
                let data = QueryData::from_db(conn, &input.sql).await?;
                expand_with_data(input, data, false)
            }
...
pub fn expand_from_file(input: QueryMacroInput, file: PathBuf) -> crate::Result<TokenStream> {
    ...
    match &*query_data.db_name {
        #[cfg(feature = "postgres")]
        rusql_core::postgres::Postgres::NAME => expand_with_data(
            input,
            QueryData::<rusql_core::postgres::Postgres>::from_dyn_data(query_data)?,
            true,
        ),
        #[cfg(feature = "mysql")]
        rusql_core::mysql::MySql::NAME => expand_with_data(
            input,
            QueryData::<rusql_core::mysql::MySql>::from_dyn_data(query_data)?,
            true,
        ),
        #[cfg(feature = "sqlite")]
        rusql_core::sqlite::Sqlite::NAME => expand_with_data(
            input,
            QueryData::<rusql_core::sqlite::Sqlite>::from_dyn_data(query_data)?,
            true,
        ),
        _ => Err(format!(
    ...
    Describe<DB>: DescribeExt,
{
        ...
        .all(|it| it.type_info().is_void())
    {
        ...
        quote! {
            ::rusql::query_with::<#db_path, _>(#sql, #query_args)
        }
    } else {

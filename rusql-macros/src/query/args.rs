...
use rusql_core::describe::Describe;
...
pub fn quote_args<DB: DatabaseExt>(
    ...
    if input.arg_exprs.is_empty() {
        return Ok(quote! {
            let query_args = <#db_path as ::rusql::database::HasArguments>::Arguments::default();
        });
    }
            ...
            params
                ...
                .map(|(i, (param_ty, (name, expr)))| -> crate::Result<_> {
                    ...
                    Ok(quote_spanned!(expr.span() =>
                        ...
                        if false {
                            use ::rusql::ty_match::{WrapSameExt as _, MatchBorrowExt as _};

                            // evaluate the expression only once in case it contains moves
                            let expr = ::rusql::ty_match::dupe_value(#name);

                            // if `expr` is `Option<T>`, get `Option<$ty>`, otherwise `$ty`
                            let ty_check = ::rusql::ty_match::WrapSame::<#param_ty, _>::new(&expr).wrap_same();

                            // if `expr` is `&str`, convert `String` to `&str`
                            let (mut _ty_check, match_borrow) = ::rusql::ty_match::MatchBorrow::new(ty_check, &expr);
    ...
    Ok(quote! {
        ...
        let mut query_args = <#db_path as ::rusql::database::HasArguments>::Arguments::default();
        query_args.reserve(
            #args_count,
            0 #(+ ::rusql::encode::Encode::<#db_path>::size_hint(#arg_name))*
        );

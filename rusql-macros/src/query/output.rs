...
fn column_to_rust<DB: DatabaseExt>(describe: &Describe<DB>, i: usize) -> crate::Result<RustColumn> {
    ...
    Ok(RustColumn {
        ...
        // https://github.com/launchbadge/rusql/issues/1322
        var_name: quote::format_ident!("rusql_query_as_{}", decl.ident),
        ident: decl.ident,
        type_,
    })
}
...
pub fn quote_query_as<DB: DatabaseExt>(
    ...
    quote! {
        ::rusql::query_with::<#db_path, _>(#sql, #bind_args).try_map(|row: #row_path| {
            use ::rusql::Row as _;
...
pub fn quote_query_scalar<DB: DatabaseExt>(
    ...
    Ok(quote! {
        ::rusql::query_scalar_with::<#db, #ty, _>(#query, #bind_args)
    })
}

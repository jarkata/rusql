...
impl Migrator {
    ...
    ///
    /// ```rust,no_run
    /// # use rusql_core::migrate::MigrateError;
    /// # fn main() -> Result<(), MigrateError> {
    /// # rusql_rt::block_on(async move {
    /// # use rusql_core::migrate::Migrator;
    /// use std::path::Path;
    ///
    ...
    /// protocol but does not support advisory locks used by RuSQL's migrations support for Postgres.
    ...
    /// let pool = rusql_core::sqlite::SqlitePoolOptions::new().connect("sqlite::memory:").await?;
    /// m.run(&pool).await
    /// #     })
    /// # }
    /// ```

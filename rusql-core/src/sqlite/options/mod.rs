...
///
/// ```rust,no_run
/// # use rusql_core::connection::ConnectOptions;
/// # use rusql_core::error::Error;
/// use rusql::sqlite::{SqliteConnectOptions, SqliteJournalMode};
/// use std::str::FromStr;
///
/// # fn main() {
/// # #[cfg(feature = "_rt-async-std")]
/// # rusql_rt::async_std::task::block_on::<_, Result<(), Error>>(async move {
...
impl SqliteConnectOptions {
    ...
    pub fn new() -> Self {
        ...
        // https://github.com/launchbadge/rusql/pull/1930#issuecomment-1168165414
        ...
        Self {
            ...
            serialized: false,
            thread_name: Arc::new(DebugFn(|id| format!("rusql-sqlite-worker-{}", id))),
    ...
    ///
    /// RuSQL chooses to enable this by default so that foreign keys function as expected,
    ...
    ///
    /// RuSQL does not set a journal mode by default, to avoid unintentionally changing a database
    /// into or out of WAL mode.
    ///
    /// The default journal mode for non-WAL databases is `DELETE`, or `MEMORY` for in-memory
    /// databases.
    ///
    /// For consistency, any commands in `rusql-cli` which create a SQLite database will create it
    /// in WAL mode.
    ...
    /// [open an issue](https://github.com/launchbadge/rusql/issues/new/choose) as this may indicate
    /// a concurrency bug in RuSQL. Please provide clear instructions for reproducing the issue,
    ...
    /// # use rusql_core::error::Error;
    /// use std::str::FromStr;
    /// use rusql::sqlite::SqliteConnectOptions;

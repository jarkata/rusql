...
impl<'q> QueryLogger<'q> {
    ...
    pub(crate) fn finish(&self) {
        ...
        if let Some(lvl) = lvl
            .to_level()
            .filter(|lvl| log::log_enabled!(target: "rusql::query", *lvl))
        {
            ...
            log::logger().log(
                &log::Record::builder()
                    ...
                    .level(lvl)
                    .module_path_static(Some("rusql::query"))
                    .target("rusql::query")
                    .build(),
            );
        }
...
impl<'q, O: Debug + Hash + Eq, R: Debug, P: Debug> QueryPlanLogger<'q, O, R, P> {
    ...
    pub(crate) fn log_enabled(&self) -> bool {
        if let Some(_lvl) = self
            ...
            .statements_level
            .to_level()
            .filter(|lvl| log::log_enabled!(target: "rusql::explain", *lvl))
        {
            return true;
        } else {
            return false;
            ...
            .filter(|lvl| log::log_enabled!(target: "rusql::explain", *lvl))
        {
            ...
            log::logger().log(
                &log::Record::builder()
                    ...
                    .level(lvl)
                    .module_path_static(Some("rusql::explain"))
                    .target("rusql::explain")
                    .build(),
            );
        }
    }

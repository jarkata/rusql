...
/// # use rusql_core::connection::{Connection, ConnectOptions};
/// # use rusql_core::postgres::{PgConnectOptions, PgConnection, PgSslMode};
/// #
/// # fn main() {
...
impl PgConnectOptions {
    ...
    ///
    /// ```rust
    /// # use rusql_core::postgres::PgConnectOptions;
    ...
    ///
    /// ```rust
    /// # use rusql_core::postgres::{PgSslMode, PgConnectOptions};
    /// let options = PgConnectOptions::new()
    ///     .ssl_mode(PgSslMode::Require);
    /// ```
    ...
    /// as of Postgres 12).
    ///
    /// RuSQL sets this value to 3 by default, which tells Postgres to return floating-point values
...
fn test_options_formatting() {
    ...
    let options = options.options([("search_path", "rusql")]);
    assert_eq!(
        options.options,
        Some("-c geqo=off -c search_path=rusql".to_string())
    );

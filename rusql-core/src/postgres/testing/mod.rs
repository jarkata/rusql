...
impl TestSupport for Postgres {
    ...
    fn cleanup_test(db_name: &str) -> BoxFuture<'_, Result<(), Error>> {
        Box::pin(async move {
            ...
            query("delete from _rusql_test.databases where db_name = $1")
                .bind(&db_name)
                .execute(&mut conn)
                .await?;
...
async fn test_context(args: &TestArgs) -> Result<TestContext<Postgres>, Error> {
    ...
    conn.execute(
        ...
        create schema if not exists _rusql_test;

        create table if not exists _rusql_test.databases (
        ...
        create index if not exists databases_created_at 
            on _rusql_test.databases(created_at);

        create sequence if not exists _rusql_test.database_ids;
    "#,
    )
    .await?;
    ...
    let new_db_name: String = query_scalar(
        r#"
            insert into _rusql_test.databases(db_name, test_path)
            select '_rusql_test_' || nextval('_rusql_test.database_ids'), $1
            returning db_name
        "#,
    )
    .bind(&args.test_path)
...
async fn do_cleanup(conn: &mut PgConnection) -> Result<usize, Error> {
    let delete_db_names: Vec<String> =
        query_scalar("select db_name from _rusql_test.databases where created_at < now()")
            .fetch_all(&mut *conn)
            .await?;
    ...
    query("delete from _rusql_test.databases where db_name = any($1::text[])")

...
impl PgAdvisoryLock {
    ...
    /// ### Example
    /// ```rust
    /// # extern crate rusql_core as rusql;
    /// use rusql::postgres::{PgAdvisoryLock, PgAdvisoryLockKey};
    ///
    ...
    pub fn new(key_string: impl AsRef<str>) -> Self {
        ...
        // exclusively to RuSQL. This should avoid collisions with implementations using a similar
        ...
        hkdf.expand(
            b"RuSQL (Rust) Postgres advisory lock",
...
impl<'lock, C: AsMut<PgConnection>> PgAdvisoryLockGuard<'lock, C> {
    ...
    /// in which case the lock will be automatically released by the connection closing anyway.
    ///
    /// If `pg_advisory_unlock()` returns `false`, a warning will be logged, both by RuSQL as

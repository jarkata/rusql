...
impl Migrate for PgConnection {
    fn ensure_migrations_table(&mut self) -> BoxFuture<'_, Result<(), MigrateError>> {
        Box::pin(async move {
            ...
            self.execute(
                r#"
CREATE TABLE IF NOT EXISTS _rusql_migrations (
    ...
    fn version(&mut self) -> BoxFuture<'_, Result<Option<(i64, bool)>, MigrateError>> {
        Box::pin(async move {
            // language=SQL
            let row = query_as(
                "SELECT version, NOT success FROM _rusql_migrations ORDER BY version DESC LIMIT 1",
            )
            .fetch_optional(self)
            .await?;
    ...
    fn dirty_version(&mut self) -> BoxFuture<'_, Result<Option<i64>, MigrateError>> {
        Box::pin(async move {
            ...
            let row: Option<(i64,)> = query_as(
                "SELECT version FROM _rusql_migrations WHERE success = false ORDER BY version LIMIT 1",
            )
            .fetch_optional(self)
            .await?;
    ...
    fn list_applied_migrations(
        ...
        Box::pin(async move {
            ...
            let rows: Vec<(i64, Vec<u8>)> =
                query_as("SELECT version, checksum FROM _rusql_migrations ORDER BY version")
                    .fetch_all(self)
                    .await?;
    ...
    fn validate<'e: 'm, 'm>(
        ...
        Box::pin(async move {
            ...
            let checksum: Option<Vec<u8>> =
                query_scalar("SELECT checksum FROM _rusql_migrations WHERE version = $1")
    ...
    fn apply<'e: 'm, 'm>(
        ...
        Box::pin(async move {
            ...
            // execute migrations twice. See https://github.com/launchbadge/rusql/issues/1966.
            ...
            let _ = query(
                r#"
    INSERT INTO _rusql_migrations ( version, description, success, checksum, execution_time )
    VALUES ( $1, $2, TRUE, $3, -1 )
                "#,
            )
            .bind(migration.version)
    ...
    fn revert<'e: 'm, 'm>(
        ...
        Box::pin(async move {
            ...
            // language=SQL
            let _ = query(r#"DELETE FROM _rusql_migrations WHERE version = $1"#)
                .bind(migration.version)
                .execute(&mut tx)
                .await?;

    ...
    fn cleanup_test(db_name: &str) -> BoxFuture<'_, Result<(), Error>> {
        Box::pin(async move {
            let mut conn = MASTER_POOL
                .get()
                .expect("cleanup_test() invoked outside `#[rusql::test]")
                .acquire()
                .await?;
            ...
            query("delete from _rusql_test_databases where db_id = ?")
                .bind(&db_id)
                .execute(&mut conn)
                .await?;
...
async fn test_context(args: &TestArgs) -> Result<TestContext<MySql>, Error> {
    ...
    conn.execute(
        r#"
        create table if not exists _rusql_test_databases (
    ...
    query("insert into _rusql_test_databases(test_path) values (?)")
        .bind(&args.test_path)
        .execute(&mut conn)
        .await?;
...
async fn do_cleanup(conn: &mut MySqlConnection) -> Result<usize, Error> {
    let delete_db_ids: Vec<u64> = query_scalar(
        "select db_id from _rusql_test_databases where created_at < current_timestamp()",
    )
    .fetch_all(&mut *conn)
    .await?;
    ...
    let mut query = QueryBuilder::new("delete from _rusql_test_databases where db_id in (");
...
fn db_name(id: u64) -> String {
    format!("_rusql_test_database_{}", id)
}

fn db_id(name: &str) -> u64 {
    name.trim_start_matches("_rusql_test_database_")
        .parse()
...
#[test]
fn test_db_name_id() {
    assert_eq!(db_name(12345), "_rusql_test_database_12345");
    assert_eq!(db_id("_rusql_test_database_12345"), 12345);
}

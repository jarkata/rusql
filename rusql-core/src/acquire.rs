...
///
/// ```rust
/// # use rusql::{Acquire, postgres::Postgres, error::BoxDynError};
...
///
///     rusql::query!("SELECT 1 as v").fetch_one(&mut *conn).await?;
///     rusql::query!("SELECT 2 as v").fetch_one(&mut *conn).await?;
///
///     Ok(())
/// }
/// ```
///
/// If you run into a lifetime error about "implementation of `rusql::Acquire` is
...
///
/// ```rust
/// # use rusql::{postgres::PgConnection, error::BoxDynError};
/// # #[cfg(any(postgres_9_6, postgres_14))]
...
///
/// [workaround]: https://github.com/launchbadge/rusql/issues/1015#issuecomment-767787777
pub trait Acquire<'c> {

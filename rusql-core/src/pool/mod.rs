//! Provides the connection pool for asynchronous RuSQL connections.
//!
...
//!
//! RuSQL provides a canonical connection pool implementation intended to satisfy the majority
//! of use cases.
//!
...
//!
//! ```rust,ignore
//! rusql::query("DELETE FROM articles").execute(&pool).await?;
//! ```
//!
...
/// An asynchronous pool of RuSQL database connections.
///
...
/// ergonomic to use as well. By design, RuSQL pushes you towards using prepared queries/statements
...
impl<DB: Database> Pool<DB> {
    ...
    /// # async fn bleh() -> rusql_core::error::Result<()> {
    /// use rusql::PgPool;
    ///
